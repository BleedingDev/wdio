describe("Mixit", () => {
  ...
  it("remove cookie bar if exist", async () => {
    const cookieBarElm = await $(
      '//div[contains(@class,"cookie-bar")]//button[contains(text(),"OK")]'
    );
    cookieBarElm.click();
  });
  ...
  it("select ready mixit", async () => {
    // main menu
    const readyMixitButton = await $(
      '//button[@aria-controls="megamenu--homepage--ready"]'
    );
    readyMixitButton.click();

    // sub menu
    const driedFruitsLinkElm = await $(
      '(//a[@href="/hotove-mixit/suche-plody"])[1]'
    );
  ...
  it("select first product", async () => {
    ...
    const heroTitleElm = await $(
      '//h1[contains(@class, "product-hero__title")]'
    );
